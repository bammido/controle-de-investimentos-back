...
export default class MovimentacoesDatabase extends BaseDatabase {
    ...
    public async createTable(): Promise<void> {
        const colunas = `papel VARCHAR(10) NOT NULL PRIMARY KEY, dataDaCompra DATE NOT NULL, corretora VARCHAR(50) NOT NULL, preco FLOAT NOT NULL, qtd FLOAT NOT NULL, tipoMovimentacao VARCHAR(10) NOT NULL, userId, FOREIGN KEY (papel) REFERENCES ${(process.env.papeis as string)} (papel), FOREIGN KEY (userId) REFERENCES ${(process.env.usuarios as string)} (id)`
